...
class HomeViewController: UIViewController , GADInterstitialDelegate {
    ...
    func updateUI(){
        height_list_genres.constant = CGFloat(list_genres.count) * HEIGHT_CELL + 50
    }
    ...
    override func viewDidLoad() {
        ...
        turnBouncesOfCollectionView(On: false)
        
    }
    
    func turnBouncesOfCollectionView(On: Bool) {
        newVideoCollectionView.bounces = On
        bieuDoCVC.bounces = On
        artistCollectionView.bounces = On
        categoryCollectionView.bounces = On
    }
    
    var isFirstLoad = true
    
    let theWidth = screenWidth
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if isFirstLoad {
            ...
            if let newVideoCollectionView = newVideoCollectionView.collectionViewLayout  as? UICollectionViewFlowLayout {
                ...
                newVideoCollectionView.itemSize.height = theWidth*0.4
                newVideoCollectionView.invalidateLayout()
                self.heightNewVideo.constant = (theWidth*0.4+10) * 10
            }
            
            if let bieuDoCVC = bieuDoCVC.collectionViewLayout as? UICollectionViewFlowLayout {
                bieuDoCVC.itemSize.width = theWidth
                bieuDoCVC.itemSize.height = theWidth*0.4
                bieuDoCVC.invalidateLayout()
                self.heightChart.constant = (theWidth*0.4+10) * 3
            }
            ...
            if let artistCollectionView = artistCollectionView.collectionViewLayout as? UICollectionViewFlowLayout {
                ...
                artistCollectionView.itemSize.height = theWidth*0.4
                artistCollectionView.invalidateLayout()
                self.heightArtist.constant = (theWidth*0.4 + 10) * 6
            }
            ...
            if let categoryCollectionView = categoryCollectionView.collectionViewLayout as? UICollectionViewFlowLayout {
                ...
                categoryCollectionView.itemSize.height = theWidth*0.4
                categoryCollectionView.invalidateLayout()
                self.heightGenres.constant = (theWidth*0.4 + 10) * 9
            }
        ...
        HomeDataStore.share.getListHome( responses: { [weak self] productHome in
            if let self = self , let productHome = productHome  {
                ...
                DispatchQueue.main.async {
                    ...
                    self.heightNewVideo.constant = (self.theWidth*0.4+10) * CGFloat(self.list_new_releases.count)
                    self.bieuDoCVC.reloadData()
                    self.heightChart.constant = (self.theWidth*0.4+10) * CGFloat(self.list_charts.count)
                    self.artistCollectionView.reloadData()
                    self.heightArtist.constant = (self.theWidth*0.4+10) * CGFloat(self.list_artists.count)
                    self.categoryCollectionView.reloadData()
                    self.heightGenres.constant = (self.theWidth*0.4+10) * CGFloat(self.list_genres.count)
                }
    ...
    func initSlideShow(){
        slideShowImageView.slideshowInterval = 3.0
